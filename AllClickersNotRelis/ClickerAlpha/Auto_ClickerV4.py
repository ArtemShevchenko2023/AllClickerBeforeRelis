...
def set_status(status):
    if paused:
        response = pyautogui.confirm(text=status, title="Уведомление", buttons=["Продолжить", "Ок"])
        ...
        response = pyautogui.confirm(text=status, title="Уведомление", buttons=["Пауза", "Ок"])
        if response == "Пауза":
            toggle_pause()
